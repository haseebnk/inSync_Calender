...
class CalendarDay extends Component {
  ...
  onDateSelected = selectedDate => {
    // Call the onDateSelected prop to update the selected date in the parent component's state
    this.props.onDateSelected(selectedDate);

    // Show the event input modal
    this.showEventModal(selectedDate);
  };
  addEventToDate = (date, event) => {
    const events = {...this.state.events};
    events[date.format('YYYY-MM-DD')] = event;
  ...
  // Function to show the event input modal
  showEventModal = selectedDate => {
    this.setState({ showModal: true, selectedDate });
  };

  // Function to hide the event input modal
  hideEventModal = () => {
    this.setState({ showModal: false, selectedDate: null });
  };
  selectAndOpen = (onDateSelected, date) => {
    onDateSelected(date);
  ...
  render() {
    ...
    let containerStyle = selected
      ...
      if (DayComponent) {
        ...
        day = (
          <TouchableOpacity onPress={() => this.selectAndOpen(onDateSelected, date)} disabled={!enabled}>
          <View
            style={[
        ...
        >
          <View style={stylesModal.centeredView}>
            <View style={stylesModal.modalView}>
              <Text style={stylesModal.label}>Add Event:</Text>
              <TextInput
              ...
              <TouchableOpacity
                ...
                onPress={() => this.hideEventModal()}
              >
                <Text style={stylesModal.addButtonText}>Add</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
